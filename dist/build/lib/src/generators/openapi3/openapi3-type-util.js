...
function unionTypeToDiscrimintorObject(unionType, typeTable) {
    ...
    const objectReferenceOnly = nonNullTypes.every(t => types_1.isReferenceType(t) && types_1.isObjectType(types_1.dereferenceType(t, typeTable)));
    ...
    const mapping = nonNullTypes.reduce((acc, t) => {
        ...
        const concreteType = types_1.dereferenceType(t, typeTable);
        // Sanity check and type cast
        if (!types_1.isObjectType(concreteType)) {
        ...
        const discriminatorProp = concreteType.properties.find(p => p.name === unionType.discriminator);
